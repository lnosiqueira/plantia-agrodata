...
def plot_media_perda_por_campo(registros: List[Dict]) -> None:
    """
    Barra: média de perda (%) por field_id.
    """
    ...
    plt.figure()  # gráfico único, sem subplots
...
def plot_serie_perda_por_campo(registros: List[Dict], field_id: int) -> None:
    """
    Linha: evolução da perda (%) no tempo para um field específico.
    """
    if not _valida_dados(registros):
        return
    ...
    for r in registros:
        try:
            if int(r["field_id"]) == int(field_id):
                ...
                loss = float(r["loss_percentage"])
                serie.append((dt, loss))
        except Exception:
            continue
